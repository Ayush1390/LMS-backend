...
//         if(!user){
//             return next(new AppError('unauthorized,login again',400));
//         }
    
//         if(user.role === 'ADMIN'){
//             return next(new AppError('ADMIN cannot purchase course',400));
//         }

//         console.log('Creating subscription for user:', id);
//         console.log('Using plan ID:', process.env.RAZORPAY_PLAN_ID);
...
//         const generatedSignature = await crypto.createHmac('sha256',process.env.RAZORPAY_SECRET).update(`${razorpay_payment_id} | ${subscriptionId}`).digest('hex');
